...
\newcommand{\codigoprojeto}{PJ151-2025}
...
\begin{titlepage}
   ...
   \includegraphics[width=0.5\textwidth]{uema.png}\\[0.3cm] 
    ...
    \textbf{PROGRAMA EXTENSÃO PARA TODOS -- PET -- EDITAL Nº 15/2025}\\
...
Durante a fase atual do projeto, foram realizadas atividades de manutenção preventiva do sistema, atualização do código-fonte para implementação de novas funcionalidades e expansão da capacidade de sensoriamento para cobertura de uma área maior do canteiro. Este relatório parcial apresenta as atividades realizadas até o momento, os resultados obtidos e as próximas etapas planejadas.
...
\begin{itemize}[leftmargin=2cm]
    \item Realizar diagnóstico completo do estado atual do circuito e identificar oportunidades de melhoria;
    \item Executar manutenção preventiva do circuito, incluindo revisão de soldas e verificação de conexões;
    \item Atualizar o código-fonte do sistema, implementando novas funcionalidades e melhorias;
...
\begin{enumerate}[leftmargin=2cm]
    ...
    \item \textbf{Diagnóstico do sistema existente:} análise detalhada do circuito e código-fonte originais, identificando oportunidades de melhoria e novas funcionalidades a serem implementadas;
    
    \item \textbf{Manutenção preventiva:} execução de revisão no circuito, incluindo verificação de soldas, teste de continuidade das conexões e inspeção dos componentes;
    
    \item \textbf{Atualização do código:} implementação de novas funcionalidades no código-fonte, incluindo suporte a múltiplos sensores e melhorias na interface do usuário;
...
A primeira etapa do trabalho consistiu na análise detalhada do sistema de irrigação desenvolvido no período anterior do projeto pela bolsista Sophia Di Consulin Almeida. O protótipo original apresentava os seguintes componentes: Arduino UNO R3, display LCD 16x2, sensor de umidade do solo FC-28, módulo relé de 1 canal, válvula solenoide 12V e fonte de alimentação.

O sistema original encontrava-se funcional e bem estruturado, demonstrando a qualidade do trabalho desenvolvido anteriormente. Durante a análise, foram identificadas oportunidades de expansão e melhorias para atender a um canteiro de maior dimensão.

\subsection{Manutenção do Circuito Eletrônico}

Com base no diagnóstico realizado, procedeu-se à manutenção preventiva do circuito. As atividades executadas incluíram:

\begin{itemize}[leftmargin=2cm]
    \item Revisão de todas as soldas para garantir a longevidade das conexões;
    \item Verificação de continuidade elétrica de todas as trilhas e conexões;
    \item Teste individual de cada componente para confirmar funcionamento adequado;
    \item Limpeza e organização da disposição dos componentes na placa.
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{sensor-umido.jpeg}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{sensor-seco.jpeg}
    \caption{Sensor de umidade FC-28 em condição de solo seco durante teste.}
    \label{fig:sensor-seco}
    \fonte{Autor, 2025.}
\end{figure}

\subsection{Atualização do Código-Fonte}

O código-fonte original foi atualizado para a versão 2.0, com a implementação de novas funcionalidades que ampliam as capacidades do sistema. A Tabela \ref{tab:comparativo-versoes} apresenta um comparativo entre as versões.
...
\begin{itemize}[leftmargin=2cm]
    \item Suporte a três sensores de umidade para cobertura de canteiro maior;
    \item Algoritmo de média aritmética das leituras para maior precisão;
    \item Conversão dos valores para porcentagem, facilitando a interpretação;
    \item Mensagem de boas-vindas ``Irriga Smart'' na inicialização;
    \item Logs detalhados via Serial Monitor para depuração e monitoramento;
    \item Documentação completa das funções no código-fonte.
\end{itemize}
...
Após a manutenção do circuito e atualização do código, foram realizados testes de funcionamento tanto em laboratório quanto no local de instalação definitiva no IEMA.

A Figura \ref{fig:lcd-umido} apresenta o display LCD exibindo a condição de solo úmido durante os testes.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{lcd-umido.jpeg}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{lcd-irrigando.jpeg}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{bomba-solenoide.jpeg}
...
\begin{enumerate}[leftmargin=2cm]
    \item \textbf{Manutenção do circuito concluída:} todas as soldas foram revisadas e as conexões verificadas, garantindo a confiabilidade do sistema;
    
    \item \textbf{Código-fonte atualizado:} o código foi atualizado para a versão 2.0, com suporte a três sensores e diversas melhorias de funcionalidade;
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{8cm}|c|}
        ...
        \hline
        Manutenção preventiva do circuito & Concluído \\
        \hline
        Atualização do código-fonte & Concluído \\
        \hline
...
Os objetivos propostos para esta etapa foram parcialmente alcançados. A manutenção do circuito eletrônico foi concluída com sucesso, garantindo a confiabilidade do sistema desenvolvido anteriormente pela bolsista Sophia Di Consulin Almeida. O código-fonte foi atualizado para a versão 2.0, implementando melhorias significativas como suporte a múltiplos sensores, exibição em porcentagem e mensagens de boas-vindas ao usuário.
...
Este projeto foi possível devido ao apoio financeiro concedido pela Universidade Estadual do Maranhão.
...
Agradeço ao professor orientador Carlos Magno Sousa Junior pela orientação e apoio durante o desenvolvimento deste trabalho. Agradeço também à bolsista anterior Sophia Di Consulin Almeida pelo excelente trabalho desenvolvido no sistema de irrigação, que serviu como base para as melhorias implementadas nesta etapa.
